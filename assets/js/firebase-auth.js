...
class FirebaseAuthHandler {
    constructor() {
        ...
        this.isSigningUp = false; // Flag to prevent race condition during signup
    }
    ...
    async init() {
        ...
        try {
            // Wait for Firebase to be available (from firebase-config.js)
            await this.waitForFirebase();

            // Get pre-initialized functions from window (exposed by firebase-config.js)
            const authFunctions = window.firebaseAuthFunctions;
            const firestoreFunctions = window.firebaseFirestore;

            if (!authFunctions || !firestoreFunctions) {
                throw new Error('Firebase functions not available. Check firebase-config.js load order.');
            }

            this.auth = window.firebaseAuthInstance;
            ...
            authFunctions.onAuthStateChanged(this.auth, async (user) => {
                this.currentUser = user;
                
                // If we are currently in the middle of a signup, DO NOT trigger profile load/creation
                // from here. The signUp method will handle it to ensure atomicity.
                if (this.isSigningUp) {
                    console.log('⏳ Auth state changed during signup - skipping automatic profile load');
                    return;
                }
            ...
            console.log('🔐 Firebase Auth initialized from global context');
    ...
    async signUp(email, password, displayName = '', phoneNumber = '') {
        ...
        this.isSigningUp = true; // Set flag to block onAuthStateChanged race condition
        try {
            ...
            // Parallelize profile operations for better speed
            const profilePromise = this.createUserProfile(user, { 
                displayName, 
                phoneNumber,
                referredBy: localStorage.getItem('pendingReferralCode') || ''
            });

            const updateAuthPromise = displayName ? updateProfile(user, { displayName }) : Promise.resolve();

            // Wait for both to complete
            const [profile] = await Promise.all([profilePromise, updateAuthPromise]);

            this.showSuccessOverlay('Account Created!', 'Welcome to Course Corner', 1200);
            
            // Explicitly sync state immediately
            this.currentUser = user;
            ...
            let msg = this.getErrorMessage(error.code);
            ...
            this.isSigningUp = false; // Reset flag
        }
    }
    ...
    async signInWithGoogle() {
        try {
            ...
            // Load or create profile in a single efficient step
            ...
            this.showSuccessOverlay('Welcome!', 'Signed in with Google', 1200);

            // Prompt users with no phone number to complete profile
            ...
            if (error.code === 'auth/unauthorized-domain') {
                this.showToast('Please add your domain to Firebase authorized domains.', 'error');
    ...
    async createUserProfile(user, additionalData = {}) {
        ...
        try {
            ...
            if (!this.db) {
                console.error('❌ Firestore DB instance is missing in FirebaseAuthHandler!');
                this.db = window.firebaseDb; // Emergency recovery
            }

            const userRef = doc(this.db, 'users', user.uid);
            console.log('📍 target path:', userRef.path);
            
            // Check if user is actually signed in from Firebase perspective
            if (!this.auth.currentUser) {
                console.warn('⚠️ No current user in auth instance during profile creation!');
            } else if (this.auth.currentUser.uid !== user.uid) {
                console.warn('⚠️ UID mismatch! Current:', this.auth.currentUser.uid, 'Target:', user.uid);
            }
            ...
            const profileData = {
                ...
                referralCreatedAt: additionalData.referralCreatedAt || null,
                referralCount: 0,
                referralPaidCount: 0,
                referralEarnings: 0,
                referralPending: 0,
            ...
            if (error.code === 'permission-denied') {
                console.error('🚫 Firestore Permission Denied. Check rules for collection "users".');
                this.showToast('Account created but profile setup failed. Please contact support.', 'warning');
            }
            throw error;
        }
    }
