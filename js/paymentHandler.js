...
class PaymentHandler {
    constructor(serverUrl = 'https://course-corner-backend.vercel.app/api') {
        ...
        try {
            ...
            const response = await fetch(`${this.serverUrl}/mpesa/stkpush`, {
                ...
                body: JSON.stringify({
                    ...
                    buyerUid: (window.firebaseAuth && window.firebaseAuth.currentUser) ? window.firebaseAuth.currentUser.uid : null
                })
            });
        ...
        try {
            ...
            const response = await fetch(`${this.serverUrl}/mpesa/verify-mpesa-code`, {
                ...
                body: JSON.stringify({ 
                    ...
                    category,
                    sessionId: this.sessionId // Pass session context
                })
            });
    ...
    // Legacy: Verify existing payment by phone or session
    async verifyExistingPayment(phoneNumber, category, onSuccess) {
        // --- NEW: Try to verify current session FIRST before asking for code ---
        if (this.sessionId) {
            try {
                const autoResult = await this.verifyPaymentManually();
                if (autoResult.success) return autoResult;
            } catch (e) {
                console.log('Background verification failed, prompting for code...');
            }
        }
    ...
    async processPayment(category, amount, onSuccess) {
        try {
            ...
            const { value: formData, isDenied } = await Swal.fire({
                ...
                preDeny: () => {
                    // Allow proceeding to verification without phone number
                    return { verify: true };
                }
            });
            ...
            if (isDenied) {
                // If verify is true, proceed to verification (no phone needed)
                if (formData && formData.verify) {
                    return await this.verifyExistingPayment(null, category, onSuccess);
                }
